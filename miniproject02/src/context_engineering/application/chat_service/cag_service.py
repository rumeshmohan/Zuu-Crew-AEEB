...
class CAGCache:
    ...
    def prewarm(self, queries: List[str], responses: Optional[List[Dict[str, Any]]] = None) -> int:
        """
...
class CAGService:
    ...
    Wraps a ``CAGCache`` with an LLM to serve answers. On each query it first
    checks the semantic cache; only on a miss does it call the LLM, then stores
    the result so future similar queries are served instantly.

    Args:
        llm: LLM client with a ``invoke`` / ``__call__`` method that accepts a
             prompt string and returns a string response.
        cache: A ``CAGCache`` instance that handles FAQ and history tiers.
        prompt_template: Optional format string with ``{query}`` and
                         ``{context}`` placeholders.  Supply ``None`` to use
    ...
    def __init__(
        ...
        if llm is None and rag_service is not None:
            llm = rag_service.llm
        if llm is None:
            raise ValueError("Either 'llm' or 'rag_service' must be provided.")
        self.llm = llm
        self.cache = cache
    ...
    def chat(self, query: str, **kwargs) -> Dict[str, Any]:
        """
        Answer a query, serving from cache when possible.
        ...
        Returns:
            ...
            - ``source`` (str): ``'faq'``, ``'history'``, or ``'llm'``.
            - ``similarity_score`` (float | None): Cache similarity if a hit.
            - ``matched_query`` (str | None): The cached query that matched.
        """
        start = time.time()
        cached = self.cache.get(query)
        if cached and cached.get('has_response', True):
            return {
        ...
        response = {'answer': answer, 'evidence_urls': []}
    ...
    def _generate(self, query: str, context: str = '') -> str:
        ...
        # Handle both plain-string and object responses (e.g. LangChain AIMessage)
    ...
    def warm_faqs(self, faq_queries: Optional[List[str]] = None, **kwargs) -> int:
        """
        Pre-generate LLM answers for all pending (un-warmed) FAQ entries.

        Args:
            faq_queries: If provided, load these questions into the FAQ tier
                         first (useful for first-time setup).

        Returns:
            Number of FAQs successfully warmed in this call.
        """
        if faq_queries:
            self.cache.load_faqs(faq_queries)
        ...
        for query in pending:
            try:
                answer = self._generate(query)
                self.cache.update_faq_response(
                    query, {'answer': answer, 'evidence_urls': []}
                )
                warmed += 1
            except Exception:
                pass  # Leave entry as pending; can be retried later
    ...
    def generate(self, query: str, **kwargs) -> Dict[str, Any]:
        """Alias for chat(). Provided for interface compatibility."""
        return self.chat(query)

    def load_faqs(self, faq_queries: List[str], responses: Optional[List[Dict[str, Any]]] = None) -> int:
        """Load FAQ questions into the cache. Alias for cache.load_faqs()."""
        return self.cache.load_faqs(faq_queries, responses)

    def clear_cache(self, clear_faqs: bool = False) -> None:
        """Clear the history (and optionally FAQ) tier of the cache."""
        self.cache.clear(clear_faqs=clear_faqs)

    def cache_stats(self) -> Dict[str, Any]:
        """Return statistics from the underlying cache."""
        return self.cache.stats()

    def list_faqs(self) -> List[Dict[str, Any]]:
        """Return all FAQ entries with their warm status."""
        return self.cache.list_faqs()


__all__ = ['CAGCache', 'CAGService']